...
public class TestApiVM extends BasePVM<ITestApiP> implements OnItemClick<TestApiEntity> {
    ...
    public void onItemClick(final int position, final TestApiEntity testApiEntity) {
        switch (position) {
            case 0:
                ...
                break;
            case 1:
                presenter.requestLastestList(new ReqCallback<List<TopicEntity>>() {
                    @Override
                    public void onReqStart() {
                        testApiEntity.setState(TestApiEntity.LOADING);
                        adapter.notifyItemChanged(position);
                ...
                break;
            case 2:
                presenter.requestTopicListByName("Livid", new ReqCallback<List<TopicEntity>>() {
                    @Override
                    public void onReqStart() {
                        testApiEntity.setState(TestApiEntity.LOADING);
                        adapter.notifyItemChanged(position);
    ...
    private void initData() {
        ...
        TestApiEntity topicsApi = new TestApiEntity(Host.BASE_URL + "topics/show.json");
        TestApiEntity nodeApi = new TestApiEntity(Host.BASE_URL + "nodes/show.json");

        List<TestApiEntity> apiEntities = new ArrayList<>();
        apiEntities.add(hotApi);
        apiEntities.add(latestedApi);
        apiEntities.add(topicsApi);
        apiEntities.add(nodeApi);
